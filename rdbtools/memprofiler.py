...
class PrintAllKeys(object):
    def __init__(self, out, bytes, largest,ttl,ttlunit):
        self._expireNoneCount=0;
        self._expireExceedCount=0;
        self._total=0;
        self._bytes = bytes
    ...
    def next_record(self, record) :
        ...
        if self._largest is None:
            if self._bytes is None or record.bytes >= int(self._bytes):
                self._total+=1
                if self._ttl is not None and record.expiry is not None and record.expiry < to_datetime(self._ttl*1000):
                    return
                expiryStr="None" 
                if self._ttl is not None:
                    if record.expiry is None:
                        #print("expiry time is None")
                        self._expireNoneCount +=1 
                    else:
                        print("expiry time %s 大于 ttl %s " % (record.expiry,to_datetime(self._ttl*1000)))
                        self._expireExceedCount+=1 

                if record.expiry is not None :
                    expiryStr=record.expiry.strftime('%Y-%m-%d %H:%M:%S')

                rec_str = "%d,%s,%s,%d,%s,%d,%d,%s\n" % (
                    record.database, record.type, record.key, record.bytes, record.encoding, record.size,
                    record.len_largest_element,expiryStr
                    )
                # rec_str = "%d,%s,%s,%d,%s,%d,%d,%s\n" % (
                #     record.database, record.type, record.key, record.bytes, record.encoding, record.size,
    ...
    def end_rdb(self):
        ...
        print("key总数为:%d,expiry为None的个数是：%d ,expiry time 超过指定时长的个数是：%d" % (self._total,self._expireNoneCount,self._expireExceedCount))
