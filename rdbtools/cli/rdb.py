...
Example : %prog --command json -k "user.*" /var/redis/6379/dump.rdb"""
    ...
    parser.add_option("--ttl", dest="ttl", default=None,
                  help="expire time still large ttl ")
    parser.add_option("--ttlunit", dest="ttlunit", default='s',
                  help="expire time still large ttl uniz default is s, s is for seconds ")
    ...
    try:
        ...
        try:
            callback = {
                ...
                'memory': lambda f: MemoryCallback(PrintAllKeys(f, options.bytes, options.largest,options.ttl,options.ttlunit),
